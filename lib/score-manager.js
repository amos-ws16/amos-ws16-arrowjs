...
function ensureValidPlugins (plugins) {
  ...
  Object.keys(plugins).forEach((key) => {
    ...
    if (!Array.isArray(plugin.inputs) && !plugin.inputGroup) {
      throw new InvalidInputError(`Plugin's inputs field is not an array: ${key}`)
    }
    if (plugin.inputGroup) {
      ensureValidInputGroups(plugin.inputGroup)
    }
  })
}

/**
 * Throws an InvalidInputError if the configuration of the inputGroups are invalid.
 *
 * @param inputGroup -  array of arrays with the specific values inside e.g.
 *                      inputGroup: [['file.title'], ['tasks.title', 'tasks[].description']]
 */
function ensureValidInputGroups (inputGroup) {
  if (!Array.isArray(inputGroup)) {
    throw new InvalidInputError(`Plugin's inputGroup field is not an array`)
  }
  for (var arr of inputGroup) {
    if (!Array.isArray(arr)) {
      throw new InvalidInputError(`Value of Plugin's inputGroup field is not an array`)
    }
  }
}
...
class ScoreManager {
  ...
  scoreWith (pluginId, blob) {
    ...
    let args = expandValues(inputValues.map(element => element[0]))
  ...
  aggregate (scores) {
    ...
    let aggregatorParseTree = aggregatorConfigParser.parse(
      this.aggregatorConfig, successfulScores)
    return aggregatorParseTree.eval()
  }
}
