...
function deleteChatMessagesNotInTimeIntervall (chat, startTime, endTime) {
  ...
  console.log('transformedChat: ' + transformedChat.chat)
  for (let chatObj of transformedChat.chat) {
    if ((chatObj.ts < startTime) || (chatObj.ts > endTime) && (chatObj.ts !== undefined) && (typeof (chatObj.ts) === 'number')) {
...
*
*/
function deleteChatMessagesNotFromUser (chat, user) {
  let transformedChat = chat
  isValidString(user)
  for (let chatObj of transformedChat.chat) {
 ...
 */
function getChatMessagesInTimeInterval (chatObject, startTime, endTime) {
  isArray(chatObject.chat)
  arrayNotEmpty(chatObject.chat)
  var transformedChat = []
  for (let message of chatObject.chat) {
    if ((typeof (message.ts) === 'undefined') || ((typeof (message.ts) === 'number') && (message.ts >= startTime) && (message.ts <= endTime))) {
      transformedChat.push(message)
    }
  }
  return {chat: transformedChat}
}
...
/**
I* returns all chat messages of specified users
* @param chatObject - an Object with a property chat which is an array of messages
* @param users - a user as text or an array of users as text array
 *
 */
function getChatMessagesFromUsers (chatObject, users) {
  isArray(chatObject.chat)
  arrayNotEmpty(chatObject.chat)
...
module.exports = { cloneObject, toDebugString, isValidString, isArray, isInRange, isInteger, isTimestamp, loadPlugin, loadModule, insertByPath, ensureValidAggregators, arrayNotEmpty, deleteChatMessagesNotInTimeIntervall, deleteChatMessagesNotFromUser, getChatMessagesInTimeInterval, getChatMessagesFromUsers }
